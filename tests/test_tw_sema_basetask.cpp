...
class Thread1 : public BaseTask {
public:
	void shutdown () {}
	virtual ~Thread1() {}
protected:
	void *work(void *d) {
		return print_and_look_timeout2( d );
	}
};

class Thread2 : public BaseTask {
public:
	void shutdown () {}
	virtual ~Thread2() {}
protected:
	void *work(void *d) {
		return print_and_count( d );
	}
};

class Thread3 : public BaseTask {
public:
	void shutdown () {}
	virtual ~Thread3() {}
protected:
	void *work(void *d) {
		return print_and_look( d );
	}
};
...
int main()
{
    ...
    /* Create independent threads each of which will execute function */
     ...
     Thread2 t2;
     Thread3 t3;
     t1.startTask(reinterpret_cast<void *>(&test_sema));
     t2.startTask(reinterpret_cast<void *>(&test_sema));
     t3.startTask(reinterpret_cast<void *>(&test_sema));
