...
char *deletekey = "2#~@@#~#!"; // something invalid for a DSRN
...
namespace TWlib { // specialize a corresponding hash function for the TWDenseHash below, needs to be defined in TWlib namespace
template<>
...
int main() {
	...
	zdeletekey = new string(deletekey);
//	string zemptykey2(emptykey, Z_DEFAULT_CP );
//	ZString zemptykey;
//	zemptykey = zemptykey2;
//	zemptykey = emptykey
	// stack test
	TWDenseHash<string, testdat, TW_Mutex, TWlibTESTS::string_eqstr, TESTAlloc> testhash( *zdeletekey, *zemptykey );

	// heap test
	TWDenseHash<string, testdat, TW_Mutex, TWlibTESTS::string_eqstr, TESTAlloc> *heaphash;
	...
	TWDenseHash<string, testdat, TW_Mutex, TWlibTESTS::string_eqstr, TESTAlloc>::HashIterator iter(testhash);
...
*/
	...
	TWDenseHash<string, testdat, TW_Mutex, TWlibTESTS::string_eqstr, TESTAlloc> *heaphashes[outerloop];

	cout << "-------Heap test - " << loop1 << " runs-----" << endl;

	for(loopb = outerloop-1; loopb >= 0; loopb--)
	for(loop = loop1-1; loop >= 0;loop--) {
		heaphashes[loopb] = new TWDenseHash<string, testdat, TW_Mutex, TWlibTESTS::string_eqstr, TESTAlloc>( *zdeletekey, *zemptykey );
